...
<div class="bgGradient">
    <div class="flight-search-container">
        ...
        <script>
            ...
            function addCity() {
                let container = document.getElementById('extra-cities');
                let newField = document.createElement('div');
                newField.innerHTML = `
                    <label>From: <input type="text" name="from[]" placeholder="Enter City"></label>
        ...
        <div class="container">
                ...
                <form method="POST">
                    ...
                    <div class="fare-options">
                        <?php foreach ($fare_options as $key => $desc): ?>
                            <input type="radio" id="fare_<?= strtolower(str_replace(' ', '_', $key)) ?>" name="fare" value="<?= $key ?>">
                            <label for="fare_<?= strtolower(str_replace(' ', '_', $key)) ?>"><?= $key ?> (<?= $desc ?>)</label>
                        <?php endforeach; ?>
                    </div>
            ...
            <div id="roundtrip-form" class="form-section">
                ...
                <form method="POST">
                    ...
                    <div class="fare-options">
                        <?php foreach ($fare_options as $key => $desc): ?>
                            <input type="radio" id="fare_<?= strtolower(str_replace(' ', '_', $key)) ?>_round" name="fare" value="<?= $key ?>">
                            <label for="fare_<?= strtolower(str_replace(' ', '_', $key)) ?>_round"><?= $key ?> (<?= $desc ?>)</label>
                        <?php endforeach; ?>
                    </div>
            ...
            <div id="multicity-form" class="form-section">
                ...
                <form method="POST">
                    ...
                    <div class="fare-options">
                        <?php foreach ($fare_options as $key => $desc): ?>
                            <input type="radio" id="fare_<?= strtolower(str_replace(' ', '_', $key)) ?>_multi" name="fare" value="<?= $key ?>">
                            <label for="fare_<?= strtolower(str_replace(' ', '_', $key)) ?>_multi"><?= $key ?> (<?= $desc ?>)</label>
                        <?php endforeach; ?>
                    </div>
